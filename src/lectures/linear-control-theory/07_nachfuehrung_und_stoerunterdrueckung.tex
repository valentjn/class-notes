...
Dieses Problem heißt \begriff{Nachführungsproblem (tracking problem)}.
...
% $\dot{\widetilde{x}} = (A - LC) \widetilde{x}$ unabhängig von $r$, d.\,h.
% der geschlossene Regelkreis ist $\smallpmatrix{\dot{x} \\ \dot{\widetilde{x}}} =
...
\smallpmatrix{x \\ d \\ u}$
ist die \begriff{erweiterte verallgemeinerte Anlage}.
